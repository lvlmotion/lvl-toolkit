...
For each session it prints what the loader sees, then calls
:func:`lvl_toolkit.driver.plot_session` to render the figures the default
schema (:func:`lvl_toolkit.graphing_schema.figures_for`) asks for and write
them next to the session -- the "download the app, run a collection, get
graphs" path, self-contained.
...
callable; write your own and pass it as ``schema=`` (see ``graphing_schema.py``
for a worked example). The renderer, colors, and overlays are reused unchanged.
"""
import sys
...
def main(argv) -> None:
    ...
    for folder in targets:
        ...
        try:
            ...
            continue
        summarize(session)
        for path in plot_session(folder):
            print(f"  wrote {path}")
